...
class AuthenticateSessionOtpController extends Controller {
	...
	public function request_otp(Request $request){
        ...
        if(!$admin)
            return redirect($redirect_url)->withError('This email address is not registered with us.');

       if (!Hash::check($request->password, $admin->password)) {
            return redirect($redirect_url)->withError('The password you entered is incorrect.');
        }
            else{
            if($admin->status == 0)

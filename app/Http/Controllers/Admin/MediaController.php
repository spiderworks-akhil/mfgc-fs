...
class MediaController extends Controller
{
	...
	// public function editor_uploads(HttpRequest $request)
    // {
    ...
    //         if($request->file('image')->move(public_path('uploads/editor'), $fileName))
    //         {
    ...
    if ($request->hasFile('image')) {
		...
		$flag = config('app.flag');
            ...
            if ($success) {
                ...
                $json["uploaded"] = false;
                $json["error"] = array("message" => "Error uploading to S3");
            }
        } else {
            if ($request->file('image')->move(public_path('uploads/editor'), $fileName)) {
                $url = asset('uploads/editor/' . $fileName); 
                ...
                $json["uploaded"] = false;
                $json["error"] = array("message" => "Error uploading file");
            }
        }
